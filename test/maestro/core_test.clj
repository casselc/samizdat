(ns maestro.core-test
  "Ported from github.com/yogthos/maestro test suite. Host interop assertions
  (.getMessage/.getCause) are rewritten to ex-message/ex-cause, which jolt
  provides; behavior contract is unchanged. One test is new here:
  async-cross-thread, which the upstream suite lacked — all its async handlers
  completed synchronously, so the executor's parking path was never exercised."
...
(deftest async
  ;; Async handlers return a deref-able — deref to get the result
  (->> @(fsm/run
...
(deftest async-cross-thread
  ;; Callback fired from a different thread after the handler has returned —
  ;; the executor must park until it arrives.
  (->> @(fsm/run
         (fsm/compile {:fsm {::fsm/start {:handler    (fn [_resources data cb _error]
                                                        (future
                                                          (Thread/sleep 20)
                                                          (cb (assoc data :x 1))))
                                          :async?     true
                                          :dispatches [[:done (constantly true)]]}
                             :done      {:handler    (fn [_resources data] (assoc data :y 2))
                                         :dispatches [[::fsm/end (constantly true)]]}}}))
       (= {:x 1 :y 2})
       (is)))
...
(deftest async-error
  (try
    ...
    (is (= 1 0))
    (catch Throwable ex
      ;; deref of a failed future wraps the exception — unwrap to find our ex-info
      (let [cause (or (ex-cause ex) ex)]
        (is (= "execution error" (ex-message cause)))))))
...
(deftest subscriptions-test
  (let [x (atom nil)]
    ;; This test has an async handler — deref the result
    @(fsm/run
      (fsm/compile {:fsm  {::fsm/start {:handler    (fn [_resources data]
                                                      (assoc data :foo :bar))
                                        :dispatches [[:foo (constantly true)]]}
...
(deftest compile-inline-preds-used-directly
  (testing "inline dispatch predicates are used directly, not evaluated as forms"
    (is (= {:ready? true}
           (fsm/run
            (fsm/compile
