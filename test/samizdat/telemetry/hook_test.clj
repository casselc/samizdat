...
(deftest lifecycle-seams-report-through-the-hook
  (let [conn (db/open! ":memory:")
        seen (atom [])]
    (try
      (hook/install! (fn [kind attrs thunk]
                       (let [v (thunk)] (swap! seen conj [kind (:status v)]) v)))
      (is (= :created (:status (lc/upsert-case! conn {:case-id "c1" :scenario-id "s" :checkpoint-id "k"
                                                       :observation-ref "o" :domain-ref "d" :spec {:a 1}}))))
      (is (= :acquired (:status (lc/acquire-lease! conn "c1" "h1" 60000))))
      (is (= :applied (:status (lc/transition! conn {:decision-id "d1" :event-id "e1" :event-type "action-proposed"
                                                      :revision 0 :case-id "c1" :evaluation-id "ev1"
                                                      :lease-holder "h1"}))))
      (is (some? (lc/decision conn "d1")))
      (is (= :released (:status (lc/release-lease! conn "c1" "h1"))))
      (is (= [[:upsert-case :created] [:acquire-lease :acquired] [:transition :applied]
              [:decision nil] [:release-lease :released]]
             @seen))
