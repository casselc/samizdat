...
(def ^:private launcher?
  "Whether a project image can be STARTED here at all.

  `image/spawn-argv` runs `jolt nrepl-server <port>`, by BARE NAME, resolved
  against the child's PATH. That is right on a developer's machine and on CI.
  It is not true inside the bounded lane's acceptance verifier, whose sandbox
  binds /usr and nothing of the host's home — so every test here crashed with
  `Cannot run program \"jolt\"` the first time this project's own suite ran as
  its own closure gate: 13 errors that were entirely about WHERE the suite was
  running and nothing about what it checks.

  A test that cannot start the thing it observes has made no observation. It
  says so rather than crashing, which is the same hygiene this suite needed
  before it could gate itself the first time (M4, d035645)."
