        ...
        (let [run-id (runs/start-run! conn {:problem "bounded smoke" :max-turns 3})
              ...
              branch (state/new-branch
                      {:id "B1" :problem "bounded smoke"
                       ;; The orientation is the FOURTH argument now: current
                       ;; upstream's third is the ROLE whose tool catalogue an
                       ;; ordinary system prompt is built from, and a bounded
                       ;; branch has no role.
                       :messages (turn/initial-messages
                                  "bounded smoke" "read only" nil
