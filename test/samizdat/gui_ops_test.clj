...
(deftest scroll-props-reset-only-on-selection-change
  ;; provenance R2-6: :scroll-top in the scrolled props on every render resets
...
(deftest a-selection-that-lost-the-fetch-race-is-refetched
  ;; provenance R2-7: when the single-flight CAS failed, the new selection's
...
(deftest an-empty-run-list-means-unknown-not-none
  ;; provenance R2-8: list-runs answers {:ok false} (not a throw) on a dead or
