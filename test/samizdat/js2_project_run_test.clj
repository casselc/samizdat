...
(deftest a-bounded-eval-never-reaches-the-ordinary-repl-image
  ;; THE CONVERGENCE REGRESSION. Current upstream routes an ordinary `eval`
  ;; to a project-rooted REPL IMAGE — a separate process with the project's
  ;; classpath and cwd, chosen per role by operator config
  ;; (samizdat.repl.route, samizdat.repl.image). That is a good thing and it
  ;; is retained; it is also emphatically not where a bounded eval belongs.
  ;;
  ;; A bounded binding's authority is a capability-bounded SCI context. If a
  ;; bounded eval reached the image it would run ordinary Clojure with the
  ;; project's classpath — every semantic operation bypassed, every receipt
  ;; unwritten, the whole boundary gone by way of a routing table.
  (when bounded?
    (let [{:keys [evaluate!]} (api)]
      (with-root [root conn]
        (seed! root)
        (let [[_ _b ev!] (execute-binding (api) conn root "routing")
              routed (atom [])]
          (with-stub-provider [launches]
            (with-redefs [route/eval-for (fn [& args] (swap! routed conj args)
                                           {:ok true :value "ORDINARY"})
                          route/doc-for (fn [& args] (swap! routed conj args) {})
                          route/complete-for (fn [& args] (swap! routed conj args) [])]
              (testing "eval evaluates in SCI, not in the image"
                (is (= 2 (:value (ev! "(+ 1 1)")))))
              (testing "and a semantic operation still reaches the provider"
                (ev! "(project/run [\"bb\" \"-M:test\"])")
                (is (= 1 @launches)))
              (is (empty? @routed)
                  (str "a bounded call reached the ordinary REPL route: "
                       (pr-str @routed))))))))))

(deftest project-run-refuses-rather-than-falling-back-to-anything
  ;; §5's other half. When the isolated environment cannot be had, the answer
  ;; is a refusal — never the ordinary eval path, never a host process, never
  ;; the VERIFY environment, never the upstream sandbox at :none.
  (when bounded?
    (let [{:keys [evaluate!]} (api)]
      (with-root [root conn]
        (seed! root)
        (let [[_ _b ev!] (execute-binding (api) conn root "nofallback")
              spawned (atom 0)
              routed (atom 0)]
          (with-redefs [pep/validate-request (fn [argv _] {:request/argv (vec argv)})
                        pep/run (fn [_ _] {:status :refused :reason :no-manager})
                        proc/run-bounded (fn [& _] (swap! spawned inc)
                                           {:status :exited :exit 0})
                        proc/run (fn [& _] (swap! spawned inc) {:exit 0 :out ""})
                        route/eval-for (fn [& _] (swap! routed inc) {:ok true})]
            (let [v (:value (ev! "(project/run [\"bb\"])"))]
              (is (= :refused (:status v)))
              (is (= :no-manager (:reason v)))
              (is (zero? @spawned) "no host process was started")
              (is (zero? @routed) "the ordinary REPL route was not consulted"))))))))
