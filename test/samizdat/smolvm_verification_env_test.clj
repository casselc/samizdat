...
(defn- machines-running?
  "Whether any smolvm machine is still up on this host — the teardown
  assertion; an ephemeral machine must be gone when its run returns."
  []
  (let [r (proc/run {:timeout-ms 20000}
                    (str (fs/which "smolvm")) "machine" "ls")]
    (boolean (re-find #"running" (str (:out r))))))
...
")
        (let [r (smve/run root ["test/ve/hang_test.clj"] 8000)]
          ...
          (Thread/sleep 300)
          (is (not (machines-running?))
              "the machine was destroyed with the process tree — nothing
