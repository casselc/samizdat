...
;; routes it to ship/bounded-done, which derives the focused argv from the
;; run's own project/edit receipts and runs it through run-bounded-verify —
;; structured argv, pinned cwd, scrubbed env, no shell. Only GREEN is
;; terminal; RED is bounded evidence and the branch continues.
  ...
  (let [bounded-ctx {:conn :fake
    ...
    (testing "GREEN: the controller-derived argv ran green => terminal"
      (let [seen (atom nil)]
        (with-redefs [estore/history (fn [_ _] (history-with "src/x.clj" "test/x_test.clj"))
                      ...
                      (fn [root argv timeout-ms]
                        (reset! seen {:root root :argv argv :timeout-ms timeout-ms})
                        {:green? true :output ""})]
          (let [r (tools/run-tool bounded-ctx)]
            ...
            (is (= "/tmp/bounded-root" (:root @seen)) "the controller pins the cwd")
            (is (= (verify/focused-argv ["src/x.clj" "test/x_test.clj"]) (:argv @seen))
                "the argv is the controller-derived focused argv, nothing else")
            (is (not-any? #{"sh" "-c"} (:argv @seen)) "no shell composes anything")))))
    ...
    (testing "a timeout reads as red evidence, not as a ship"
      (with-redefs [estore/history (fn [_ _] (history-with "test/x_test.clj"))
                    verify/run-bounded-verify (fn [_ _ _] {:green? false :timeout? true :output ""})]
        (let [r (tools/run-tool bounded-ctx)]
          (is (not (:done? r)))
    ...
    (testing "no edits at all => refused as hollow, and nothing is spawned"
      (let [ran (atom false)]
        (with-redefs [estore/history (fn [_ _] [])
                      verify/run-bounded-verify (fn [& _] (reset! ran true)
                                                  {:green? true :output ""})]
          (let [r (tools/run-tool bounded-ctx)]
            (is (not (:done? r)))
    ...
    (testing "a binding with no recordable history fails closed"
      (with-redefs [verify/run-bounded-verify (fn [& _] {:green? true :output ""})]
