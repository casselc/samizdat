...
(deftest concurrent-executions-clean-up-only-their-own-machines
  ;; THE CROSS-RUN SAFETY TEST, and the reason invocation-owned cleanup
  ;; exists. The frozen JS2 implementation swept every ephemeral machine the
  ;; manager held, which is indistinguishable from correct while exactly one
  ;; execution runs at a time — and is cross-run interference the moment a
  ;; server has two. A times out; B is a different run's execution, still
  ;; legitimately running; A's cleanup must not touch it.
  ;;
  ;; The ownership evidence is explicit rather than implied: each result names
  ;; the machine the manager started for it, and the assertion is that A's
  ;; cleanup acted on A's machine and on nothing else — not merely that B
  ;; happened to survive.
  (when @substrate?
    (with-project [root]
      (let [b-result (promise)
            ;; B first, and given room to still be running when A's deadline
            ;; lands: A waits 12s, B works for 40 and is allowed 120.
            b (future (deliver b-result
                               (run! root ["/bin/sh" "-c" "sleep 40; echo B-DONE"]
                                     {:timeout-ms 120000})))]
        (try
          (Thread/sleep 4000)                       ; let B's machine boot
          (let [a (run! root ["/bin/sh" "-c" "sleep 600"] {:timeout-ms 12000})
                bb (deref b-result 180000 ::b-never-finished)]
            (testing "A timed out and hard-cleaned"
              (is (= :timeout (:status a)))
              (is (= :hard-cleaned (:disposition a)))
              (is (string? (:machine a)) "the manager named A's machine"))

            (testing "B is a real, independent, still-running execution"
              (is (not= ::b-never-finished bb))
              (is (= :completed (:status bb)))
              (is (= 0 (:exit bb)))
              (is (str/includes? (get-in bb [:stdout :text]) "B-DONE"))
              (is (string? (:machine bb)))
              (is (not= (:machine a) (:machine bb))
                  "two executions are two machines"))

            (testing "A's cleanup acted on A's machine and on NOTHING else"
              (let [acted (set (get-in a [:cleanup :acted]))
                    owned (set (get-in a [:cleanup :owned]))]
                (is (= #{(:machine a)} owned)
                    (str "A owned exactly its own machine; got " (pr-str owned)))
                (is (= #{(str "stop " (:machine a)) (str "delete " (:machine a))}
                       acted)
                    (str "A must stop and delete only its own machine; acted on "
                         (pr-str acted)))
                (is (not-any? #(str/includes? % (str (:machine bb))) acted)
                    "B's machine was never a cleanup target")))

            (testing "A's machine is gone and B's was never stopped"
              (is (true? (get-in a [:cleanup :clean?])))
              (let [table (str (:out (proc/run {:timeout-ms 15000}
                                               "smolvm" "machine" "ls")))]
                (is (not (str/includes? table (str (:machine a))))
                    "A's machine is not in the manager's table")))

            (testing "the invocation counters stay truthful"
              (is (not= (:invocation a) (:invocation bb)))
              (is (pos? (:invocation a)))
              (is (pos? (:invocation bb))))

            (testing "each result carries its OWN identity"
              (is (= (spe/environment-coordinate) (:environment a)))
              (is (= (spe/environment-coordinate) (:environment bb)))
              (is (= (:input a) (:input bb))
                  "same tree, same staged input coordinate")
              (is (not= (:argv a) (:argv bb))))

            (testing "the poison did not outlive A's own cleanup"
              (is (false? (spe/poisoned?)))))
          (finally (future-cancel b)))))))

(deftest two-successful-concurrent-executions-do-not-interfere
  (when @substrate?
    (with-project [root]
      (let [a (future (run! root ["/bin/sh" "-c" "sleep 3; echo AAA"]))
            b (future (run! root ["/bin/sh" "-c" "sleep 3; echo BBB"]))
            ra (deref a 180000 ::never) rb (deref b 180000 ::never)]
        (is (= :completed (:status ra)))
        (is (= :completed (:status rb)))
        (is (str/includes? (get-in ra [:stdout :text]) "AAA"))
        (is (str/includes? (get-in rb [:stdout :text]) "BBB"))
        (is (not= (:machine ra) (:machine rb)))
        (is (not= (:invocation ra) (:invocation rb)))
        (is (= :terminated (:disposition ra)))
