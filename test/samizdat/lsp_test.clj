...
(deftest concurrent-requests-correlate-their-own-responses
  ;; RFC-000 CR1-4: every caller read the shared stream itself, so
...
(deftest client-for-starts-one-client-per-root-under-race
  ;; RFC-000 R2-9: client-for was get-then-start, so two branches sharing a
...
(deftest a-late-diagnostics-caller-does-not-erase-a-waiting-caller-s-push
  ;; RFC-000 R2-10: diagnostics dissoc'd the uri at entry, so a second caller
...
(deftest a-dead-reader-releases-waiters-and-evicts-the-client
  ;; RFC-000 R2-17: the reader's routing ran outside any guard, so a throw
