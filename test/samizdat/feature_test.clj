...
(deftest supervisor-forces-a-round-when-no-implementor-ships
  ;; Reviewer PASSes, so without the supervisor the run would ship round 0. But
  ;; the implementors exhaust (ship nothing), so :feature/supervise escalates and
  ;; route sends it back — proving the supervisor tweaks the outer loop.
  (with-redefs [judge/deterministic-block (constantly nil)
                judge/parse-verdict (constantly :complete)
                judge/blocking-findings (constantly nil)
