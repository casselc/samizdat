...
set -eu
cd /home/chuck/opencode/src/js2-evidence
RID=${JS2_RUN_ID:?set JS2_RUN_ID}
TGT=/home/chuck/opencode/src/samizdat-canary-target-js2
SUFFIX=${1:-1}
echo "run-id: $RID"
curl -s -m 30 "http://127.0.0.1:3993/v1/runs/$RID" > "pre-interrupt-run-$SUFFIX.json"
python3 - "$SUFFIX" <<'PY'
import json, sys
...
python3 -c "
import sqlite3
c=sqlite3.connect('file:js2-canary.sqlite3?mode=ro',uri=True)
...
git -C /home/chuck/opencode/src/samizdat-controller-js2 rev-parse HEAD
git -C /home/chuck/opencode/src/samizdat-controller-js2 status --porcelain --untracked-files=no | wc -l
