#!/bin/sh
# JS2 CONVERGENCE CLOSURE smoke controller launcher.
#
# Runs the PUSHED closure executable oracle as the harness process, with a
# separate disposable target checkout as :run :root. Nothing here is committed
# into the controller tree; the controller checkout stays byte-identical to its
# SHA, and that SHA is the one on origin.
#
...
TARGET=/home/chuck/opencode/src/js2-closure-target
...
HARNESS_ROOT="$TARGET"
HARNESS_DB="$EV/js2-closure.sqlite3"
HARNESS_MAX_TURNS=60
HARNESS_BEAM_WIDTH=1
HARNESS_PORT=3995
HARNESS_NREPL_PORT=7895
HARNESS_BUDGET_CEILING=200
HARNESS_BUDGET_PRINCIPAL=js2-closure-operator
HARNESS_BUDGET_TOKEN=$(cat "$EV/.budget-token")

# The controller's acceptance environment.
SAMIZDAT_VERIFY_ENV=bwrap
# The model's development environment: the pinned guest image, by digest. The
# same bytes bin/js2 check pinned, referenced rather than copied.
SAMIZDAT_SMOLVM_IMAGE=/home/chuck/opencode/src/js2-converge-evidence/worker-image.tar
