  ...
  (if-let [{:keys [abort]} (get @active run-id)]
    (do (reset! abort true)
        (if (pos? (runs/finish-run! conn run-id :aborted nil))
          ...
          ;; the row guard refused the rewrite (RFC-000 R2-4). Same refusal
    ...
    (let [llm-config (run-llm-config (:llm config) body)
      ...
      (future
        (try
          ...
          ;; reason as start-run! (RFC-000 CR1-3): an assoc on the
...
(defn intervene!
  ...
  git push, curl, installs) blocked a run forever (RFC-000 A-2, docs/RFCS/RFC-000-index.md)."
  [conn run-id body]
  (if (= "grant" (:kind body))
    ...
    (if-not (contains? interventions/kinds (:kind body))
      ;; RFC-000 R3-12: this reached submit!'s throw and surfaced as the
