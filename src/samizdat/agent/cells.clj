...
;; https://www.eclipse.org/licenses/epl-2.0/
;;
;; SPDX-License-Identifier: EPL-2.0

(ns samizdat.agent.cells
  "The loop's turn steps as mycelium cells.

  Each cell is a thin wrapper over the named step in samizdat.agent.loop, so
  the manifest-driven driver and the beam's run-turn share one implementation
  of every step (karamazov-ioo.20 removes the second driver). The workflow
  data map carries {:branch :turn} plus the per-turn products (:call :parsed
  :signals :said :result :tool :verdict); resources carry the run ctx
  ({:conn :run-id :config :llm-adapter :llm-config :max-turns}).

  Naming is load-bearing: the :llm/*, :tool/*, :journal/*, :gate/* namespaces
  are what glob-scoped interceptors match on, so journaling and permission
  seams apply to every future cell in those families automatically."
...
(defn register!
  "Register (or re-register) every loop cell. Called at namespace load and
  again by the workflow compiler: the cell registry is global mutable state —
  anything from a REPL clear to a test fixture can empty it — and compiling
  the loop against a half-empty registry must be impossible."
  []
  ...
  (cell/defcell :llm/infer
    {:doc "One model call, retried once at a doubled budget when the response
          hit the token cap before emitting a tool call. Produces :call
          {:ok :response} or {:ok false :error} — a provider failure is data,
          never an exception."
     :effects [:net :db]}
  ...
  (cell/defcell :llm/parse
    {:doc "Fold the response into the branch: parse the fence, record mechanics
          signals, append what the assistant actually said. On a provider
          failure this passes the data through untouched for the error route."
     :pure true}
  ...
  (cell/defcell :journal/record
    {:doc "The durable record of the turn: the turn row, any artifact and its
          entry into the shared pool, any failure, any thesis. Everything a
          gate reads and everything resume replays goes through here."
     :effects [:db]}
      ...
      (let [max-turns (:max-turns ctx)
        ...
        (cond-> (assoc data :verdict verdict)
          ...
          (-> (update :turn inc)
              ...
              ;; quadratically over a run. The journal is the durable record;
              ;; the in-data trace is a debugging window, and a window has edges.
