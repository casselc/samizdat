...
(ns samizdat.lsp.client
  ...
  (RFC-000 CR1-4). Each request is bounded by a timeout so a
...
(defn- start-reader!
  ...
  used to kill the loop with waiters still parked, RFC-000 R2-17) — releases
  ...
  (future
    (loop [alive? true]
      (let [msg (try (read-frame client)
        ...
        (when (and msg ok?)
          (recur true))))
    ;; any exit: release every waiter, evict the corpse (RFC-000 R2-17)
...
(defn- store-diagnostics! [client msg]
  (when (= "textDocument/publishDiagnostics" (:method msg))
    (let [uri (get-in msg [:params :uri])
          ...
          ;; request from one that predates it (RFC-000 R2-10).
...
(defn diagnostics
  ...
  (RFC-000 R2-10) — another caller on the same file cannot erase what this one
